...
export default function LaporanDanPddiktiPage() {
  ...
  // Handler: Start Live Entity Sync (push = kirim data lokal ke PDDIKTI, pull = tarik data dari PDDIKTI ke lokal)
  const handleStartSync = (entity: FeederEntity | null, direction: 'push' | 'pull' = 'push') => {
    ...
    setTimeout(() => {
      ...
      setSyncLogsProgress((prev) => [
        ...
        direction === 'push'
          ...
          : `[${new Date().toLocaleTimeString()}] Meminta daftar record terbaru dari server PDDIKTI...`,
        direction === 'push'
          ? `[${new Date().toLocaleTimeString()}] Mengemas payload JSON untuk transmisi batch...`
          : `[${new Date().toLocaleTimeString()}] Membandingkan record PDDIKTI dengan data lokal...`,
      ]);
    }, 800);
    ...
    setTimeout(() => {
      ...
      setSyncLogsProgress((prev) => [
        ...
        direction === 'push'
          ...
          : `[${new Date().toLocaleTimeString()}] Menarik data dari server PDDIKTI live2.php...`,
    ...
    setTimeout(() => {
      ...
      setSyncLogsProgress((prev) => [
        ...prev,
        direction === 'push'
          ? `[${new Date().toLocaleTimeString()}] SELESAI: Seluruh record berhasil didorong (push) ke server PDDIKTI.`
          : `[${new Date().toLocaleTimeString()}] SELESAI: Seluruh record berhasil ditarik (pull) dari server PDDIKTI.`,
      ]);
      ...
      const newLog: SyncLogItem = {
        ...
        message:
          direction === 'push'
            ? 'Batch berhasil didorong (push) ke Web Service Neo Feeder.'
            : 'Batch berhasil ditarik (pull) dari Web Service Neo Feeder.',
      };
      ...
      showToast(
        direction === 'push'
          ? 'Data lokal berhasil didorong (push) ke Neo Feeder PDDIKTI.'
          : 'Data terbaru berhasil ditarik (pull) dari Neo Feeder PDDIKTI.',
      );
    }, 2400);
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <button
              ...
              disabled={isLoading}
              title="Tarik data terbaru dari server PDDIKTI ke lokal"
              className="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-bold text-[#1E3A8A] bg-blue-50 border border-blue-200 hover:bg-blue-100 rounded-xl shadow-xs transition-all active:scale-[0.98] cursor-pointer disabled:opacity-60"
            >
            ...
            <button
              ...
              disabled={isLoading}
              title="Dorong seluruh data lokal ke server PDDIKTI"
            ...
            >
              <ArrowUpFromLine className="w-4 h-4 text-[#D4A017]" />
                        ...
                        <tr key={entity.id} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-3.5 px-4 text-center">
                            <div className="inline-flex items-center gap-1">
                              <button
                                onClick={() => handleStartSync(entity, 'pull')}
                                className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-bold text-[#1E3A8A] bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-all active:scale-[0.98] cursor-pointer"
                                title="Tarik data entitas ini dari PDDIKTI ke lokal"
                              >
                                <ArrowDownToLine className="w-3 h-3" />
                                <span>Pull</span>
                              </button>
                              <button
                                onClick={() => handleStartSync(entity, 'push')}
                                className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-bold text-white bg-[#1E3A8A] hover:bg-blue-900 rounded-lg shadow-xs transition-all active:scale-[0.98] cursor-pointer"
                                title="Dorong data entitas ini ke PDDIKTI"
                              >
                                <ArrowUpFromLine className="w-3 h-3 text-[#D4A017]" />
                                <span>Push</span>
                              </button>
                            </div>
                          </td>
