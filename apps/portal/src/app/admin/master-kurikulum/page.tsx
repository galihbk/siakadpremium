...
export default function AdminMasterKurikulumPage() {
  const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1';

  const [curriculums, setCurriculums] = useState<CurriculumItem[]>([]);
  const [studyProgramOptions, setStudyProgramOptions] = useState<{ id: string; name: string; facultyCode: string; degreeLevel: string }[]>([]);
  const [facultyOptions, setFacultyOptions] = useState<{ code: string; name: string }[]>([]);
  ...
  useEffect(() => {
      ...
      .then((json) => {
        if (Array.isArray(json?.data)) {
          setStudyProgramOptions(
            json.data.map((p: any) => ({ id: p.id, name: p.name, facultyCode: p.facultyCode, degreeLevel: p.degreeLevel })),
          );
        }
      })
      .catch(() => {});
    fetch(`${apiBase}/faculties`)
      ...
      .then((json) => {
        if (Array.isArray(json?.data)) {
          setFacultyOptions(json.data.map((f: any) => ({ code: f.code, name: f.name })));
        }
      })
      .catch(() => {});
  ...
  return (
    <PortalLayout
      ...
      activeMenuHref="/admin/master-kurikulum"
    >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1">
              ...
              <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
              <span className="text-[#1E3A8A] font-bold">Master Kurikulum</span>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const sel = studyProgramOptions.find((p) => p.name === e.target.value);
                        setFormData({
                          ...formData,
                          studyProgram: e.target.value,
                          facultyCode: sel?.facultyCode || formData.facultyCode,
                          degreeLevel: (sel?.degreeLevel as CurriculumItem['degreeLevel']) || formData.degreeLevel,
                        });
                      }}
