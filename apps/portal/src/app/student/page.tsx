...
export default function StudentDashboardPage() {
  ...
  // KRS status: DRAFT (belum bayar UKT) -> SUBMITTED (menunggu ACC PA) -> APPROVED (disetujui)
  ...
  const hasUnpaidDraft = enrollments.some((e: any) => e.status === 'DRAFT');
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-subtle">
            ...
            {allApproved ? (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>DISETUJUI DOSEN PA</span>
              </div>
            ) : hasSubmitted ? (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200">
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>MENUNGGU VALIDASI PA</span>
              </div>
            ) : hasUnpaidDraft ? (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200">
                <CreditCard className="w-3.5 h-3.5" />
                <span>MENUNGGU PEMBAYARAN UKT</span>
              </div>
            ) : (
