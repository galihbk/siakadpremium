...
import { FileSignature, DoorOpen, RefreshCw, ChevronRight, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function LecturerKontrakPage() {
            ...
            list.map(async (c) => {
              try {
                ...
                if (r.ok) {
                  ...
                  return [c.id, Boolean((j.data || j)?.contract)] as const;
                }
              } catch {}
              return [c.id, false] as const;
            }),
          );
          setContractStatus(Object.fromEntries(statuses));
        }
      } catch (err) {
        console.error(err);
      } finally {
      ...
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-[#1E3A8A] to-[#1E40AF] rounded-2xl p-6 sm:p-8 text-white shadow-sm">
          ...
          <p className="text-xs sm:text-sm text-blue-200 mt-1">
            Susun rencana pembelajaran semester untuk tiap kelas yang Anda ajar.
          </p>
        </div>
